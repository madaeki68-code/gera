...
const Login: React.FC<LoginProps> = ({ onLoginSuccess, users }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        // Login successful - the auth provider will handle the rest
    ...
    setTimeout(() => {
      ...
      if (!users || users.length === 0) {
        setError("Silakan import data ke database Supabase terlebih dahulu.");
        setIsLoading(false);
        return;
      }
      ...
      if (user) {
        ...
        setError("Username atau kata sandi salah. Coba: admin@vena.pictures / admin");
      }
      setIsLoading(false);
    }, 1000);
  };
        ...
        <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-slate-500 mt-2">
              Hey, masukkan detail Anda untuk masuk ke akun Anda
            </p>
          </div>
